...
function BuilderWorkspace() {
    ...
    // --- DOM-AWARE PDF GENERATION FUNCTION ---
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            // 1. Capture the DOM element as one giant Canvas
            ...
            // 2. Initialize jsPDF (A4 standard: 210mm x 297mm)
            const pdf = new jsPDF('p', 'mm', 'a4');
            const pdfWidth = pdf.internal.pageSize.getWidth();
            const pdfHeight = pdf.internal.pageSize.getHeight();

            // 3. Calculate scales: Map CSS pixels to PDF mm
            const cssScale = pdfWidth / previewEl.offsetWidth; // 1px in CSS = cssScale in mm
            const pageHeightInCssPx = pdfHeight / cssScale;    // How many CSS pixels fit on one A4 page
            const imgHeightInMm = (canvas.height * pdfWidth) / canvas.width; // Total scaled height in mm

            // 4. Find all protected elements mapping their top/bottom coordinates relative to the preview wrapper
            ...
            // 5. Smartly loop to cut the pages 
            while (currentCssY < totalCssHeight) {
                let nextCssY = currentCssY + pageHeightInCssPx;

                // Check if the natural cut falls inside any protected element
                if (nextCssY < totalCssHeight) {
                    for (const rect of avoidRects) {
                        if (nextCssY > rect.top && nextCssY < rect.bottom) {
                            // Cut falls inside! Adjust cut line to the top of this element (with 15px padding)
                            const safeBreak = rect.top - 15;

                            // Fallback: If element is bigger than a page, we must force a hard break
                            if (safeBreak <= currentCssY) {
                                nextCssY = currentCssY + pageHeightInCssPx;
                            } else {
                                nextCssY = safeBreak;
                            }
                            break;
                ...
                // 6. Draw the canvas, shifting it up by current Y position
                const yOffsetMm = -(currentCssY * cssScale);
                pdf.addImage(imgData, 'PNG', 0, yOffsetMm, pdfWidth, imgHeightInMm);

                // 7. White-out the bottom of the page if we broke early to hide the next element
                                    ...
                                    {days.map((day, index) => (
                                        <div key={index} className="flex gap-8">
                                            <div className="flex flex-col items-center">
                                                ...
                                                {/* Notice the line isn't protected, letting it naturally bridge pages seamlessly */}
                                                {index !== days.length - 1 && <div className="w-0.5 h-full bg-gray-200 mt-4"></div>}
                                            </div>
                                            <div className="pt-1 w-full">

                                                {/* --- AVOID BREAK FOR DAY HEADER --- */}
                                                <div className="avoid-break mb-6">
                                                    <h3 className="text-2xl font-serif font-bold text-gray-900 mb-4">{day.dayTitle || `Day ${day.dayNumber}`}</h3>
                                                    {day.dayDescription && (
                                                        <p className="text-gray-600 text-sm leading-relaxed italic border-l-2 border-amber-400 pl-4 py-1 bg-amber-50/30">
                                                            {day.dayDescription}
                                                        </p>
                                                    )}
                                                </div>
